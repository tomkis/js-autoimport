...
export default class ImportRepository {
  ...
  pushSuggestedImport(suggestedImport, currentlyEditedFilePath) {
    ...
    let filePromise;
    if (suggestedImport.files.length > 1) {
      filePromise = this.picker.show(suggestedImport.files, suggestedImport.text);
    } else {
      filePromise = new Promise((resolve) => {
        resolve(suggestedImport.files[0]);
      });
    }

    filePromise.then((file) => {
      const suggestedImportFile = path.relative(path.dirname(currentlyEditedFilePath), file);
      ...
      if (suggestedImportFile !== path.basename(currentlyEditedFilePath))  {
        ...
        if (existingImportFileIndex !== -1) {
          if (!_.contains(_.pluck(this.imports[existingImportFileIndex].specifiers, 'textImported'), suggestedImport.text)) {
            if (!this.hasExistingLocalImportWithSameName(existingImportFileIndex, suggestedImport.text)) {
              ...
              //TODO: rename import when duplicated
            }
          }
        } else {
          if (!this.hasExistingLocalImportWithSameName(suggestedFileInFolder, suggestedImport.text)) {
            this.imports.push({
  ...
  hasExistingLocalImportWithSameName(file, localImport) {
    return _.some(this.imports, (singleImport) => {
      return singleImport.file !== file && _.contains(_.pluck(singleImport.specifiers, 'textImported'), localImport);
    });
  }
