  ...
  pushSuggestedImport(suggestedImport, currentlyEditedFilePath) {
    ...
    if (suggestedImportFile !== path.basename(currentlyEditedFilePath))  {
      ...
      if (existingImportFileIndex !== -1) {
        if (!_.contains(this.imports[existingImportFileIndex].specifiers, suggestedImport.text)) {
          this.imports[existingImportFileIndex].specifiers.push(suggestedImport.text);
          this.changeCb(this.imports);
          this.suggested = true;
        }
      } else {
        this.imports.push({
          ...
          specifiers: [suggestedImport.text]
        });
        this.changeCb(this.imports);
        this.suggested = true;
      }
