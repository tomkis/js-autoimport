    ...
    return groupedImports[LIBRARY_IMPORT]
          ...
          .map((singleImport, row) => {
            ...
            if (singleImport.specifiers.length > 0) {
              const defaultSpecifiers = _.pluck(_.filter(singleImport.specifiers, (specifier) => specifier.def), 'text');
              const namedSpecifiers = _.pluck(_.filter(singleImport.specifiers, (specifier) => !specifier.def), 'text').sort();
              const leftBracketWithComma = defaultSpecifiers.length === 1 ? ', { ' : '{ ';
              const leftBracket = namedSpecifiers.length > 0 ? leftBracketWithComma : '';
              const rightBracket = namedSpecifiers.length > 0 ? ' }' : '';
              const defaultSpecifier = defaultSpecifiers.length === 1 ? defaultSpecifiers[0] : '';

              return `import ${defaultSpecifier}${leftBracket}${namedSpecifiers.join(', ')}${rightBracket} from '${singleImport.file}'`;
            } else {
